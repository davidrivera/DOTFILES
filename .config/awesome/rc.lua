...
config_dir  = ("/home/david/.config/awesome/")
themes_dir  = (config_dir .. "/themes")
wall_dir    = ("/home/david/Pictures/wallpapers/")
...
musicplr      = "x-terminal-emulator -t Music --geometry 130x34-320+16 -e ncmpcpp"
--iptraf        = "lilyterm -T 'IP traffic monitor' -g 180x54-20+34 -e sudo iptraf-ng -i all"
iptraf        = "x-terminal-emulator -T 'IP traffic monitor' --geometry=180x54-20+34 -e sudo iptraf -i all"
mailmutt      = "lilyterm -T 'Mutt' -g 140x44-20+34 -e mutt"
gmail         = "google-chrome --app=http://www.gmail.com"
...
myawesomemenu = {
  {"edit config",           "terminal -x vim /home/david/.config/awesome/rc.lua"},
  {"edit theme",            "terminal -x vim /home/david/.config/awesome/themes/powerarrow/theme.lua"},
...
docsmenu = {
  {" C",                    "/home/david/Tools/doc_c", beautiful.c_icon},
  {" JavaScript",           "/home/david/Tools/doc_js", beautiful.js_icon},
  {" Ruby",                 "/home/david/Tools/doc_ruby", beautiful.ruby_icon} 
}

learningmenu = {
  {" C",                    "/home/david/Books/C.sh", beautiful.c_icon},
  {" JavaScript",           "/home/david/Books/JavaScrip.sh", beautiful.js_icon},
  {" Ruby On Rails",        "/home/david/Books/RubyOnRails.sh", beautiful.ruby_icon}
}
...
myedumenu = {
  ...
  {" Free42dec",            "/home/david/Tools/Free42Linux/gtk/free42dec", beautiful.free42_icon},
  ...
  {" Wolfram Mathematica",  "/home/david/Tools/Wolfram/Mathematica", beautiful.mathematica_icon},
  {" XMind",                "xmind", beautiful.xmind_icon}
}

mydevmenu = {
  {" Android SDK Updater",  "android", beautiful.android_icon},
  {" Eclipse",              "/home/david/Tools/eclipse/eclipse", beautiful.eclipse_icon},
  {" Emacs",                "emacs", beautiful.emacs_icon},
  {" GHex",                 "ghex", beautiful.ghex_icon},	
  {" IntellijIDEA",         "/home/david/Tools/idea-IU-123.72/bin/idea.sh", beautiful.ideaUE_icon},
  ...
  {" RubyMine",             "/home/david/Tools/rubymine.run", beautiful.rubymine_icon},
...
mygraphicsmenu = {
  ...
  {" XnView",               "/home/david/Tools/XnView/xnview.sh", beautiful.xnview_icon}
}
...
mymultimediamenu = {
  ...
  {" Puddletag",            "puddletag", beautiful.puddle_icon}
}
...
mywebmenu = {
  {" Chromium",             "google-chrome", beautiful.chromium_icon},
  ...
  {" Tor",                  "/home/david/Tools/tor-browser_en-US/start-tor-browser", beautiful.vidalia_icon},
...
mytoolsmenu = {
  ...
  {" TeamViewer",           "/home/david/Tools/teamviewer7/teamviewer", beautiful.teamviewer_icon},
...
function check_mail(count)
    if count > 0 then
        return image(beautiful.widget_mail_new)
    else
        return image(beautiful.widget_mail)
    end
end
mailicon = widget ({type = "imagebox" })
mailicon.image = image(beautiful.widget_mail)
mailicon:buttons(awful.util.table.join(awful.button({ }, 1, 
function () awful.util.spawn_with_shell(gmail) end)))
mygmail = widget({ type = "textbox" })
gmail_t = awful.tooltip({ objects = { mygmail }, })
vicious.register(mygmail,  vicious.widgets.gmail, 
    function (widget,  args)
    mailicon.image = check_mail(args["{count}"])
...
musicwidget = awesompd:create() -- Create awesompd widget
  ...
  musicwidget.path_to_icons = "/home/david/.config/awesome/awesompd/icons" 
  ...
  musicwidget.mpd_config = "/home/david/.mpd/mpd.conf"
...
'<span background="#4B696D" font="Terminus 12"> <span font="Terminus 9" color="#DDDDDD"> $1/$2% <span color="#888888">·</span> $3/$4% </span></span>', 3)
cpuicon = widget ({type = "imagebox" })
cpuicon.image = image(beautiful.widget_cpu)
sensors = widget({ type = "textbox" })
--vicious.register(sensors, vicious.widgets.thermal,  " - $1°C",  20,  { "coretemp.0",  "core"} )
function c_to_f (a)
    return ((a*1.8)+32) 
end
--(high = +87.0°C/188.6F,  crit = +105.0°C/221F))
crit = 105
high = 87
tempicon = widget ({type = "imagebox" })
function temp_color_c(a)
    if a < high then
        return image(beautiful.widget_temp_good)
        --return a .. '<span color="#A4C639">°F</span>'
    elseif a >= high and a < crit then
        return image(beautiful.widget_temp_high)
        --return '<span color="#FFF00">' .. a .. '°F</span>'
    elseif a >= crit then
        return image(beautiful.widget_temp_crit)
        --return '<span color="#FF0000">' .. a .. '°F</span>'
    else return image(beautiful.widget_temp)
    end 
end
vicious.register(sensors, vicious.widgets.thermal, 
    function (widget, args)
        if args[1] > 0 then
            tzfound = true
            tempicon.image = temp_color_c(args[1])
            return "<span font='Terminus 12' background='#4b3b51'> <span font='Terminus 9' color='#DDDDDD'>" .. c_to_f(args[1]) .. "°F </span></span>"
        else return "" 
        end
    end
    , 19, {"coretemp.0","core"})
...
vicious.widgets.net,
'<span background="#C2C2A4" font="Terminus 12"> <span font="Terminus 9" color="#FFFFFF">${wlan0 down_kb}kb ↓↑ ${wlan0 up_kb}kb</span> </span>', 3)
...
function wifiInfo(adapter)
    ...
    local ws = f:read('*all'); 
    local numDOT = string.match(ws,  '[%d]+[%.]'); 
    local num = 0; 
    if numDOT == nil then
        num = 0; 
    else
        num = string.match(numDOT, '[%d]+'); 
    end

    ws = tonumber(num); 
    if ws == nil then
        return image(beautiful.widget_net)
    elseif ws < 50 then
        return image(beautiful.widget_net_bad)
    elseif ws < 60 and ws > 50 then
        return image(beautiful.widget_net_ok)
    elseif ws > 61 and ws < 80 then
        return image(beautiful.widget_net_good)
    elseif ws > 80 then
        return image(beautiful.widget_net_very_good)
    else
        return image(beautiful.widget_net_scan)
    end
end
--neticon.image = wifiInfo('wlan0')
awful.hooks.timer.register(2, function()
    neticon.image = wifiInfo("wlan0")
    --neticon.image = image(beautiful.widget_net_scan)
end)
--{{---| Binary Clock |-----------------------------------------------------------------------------
...
--{{---| Calendar widget |--------------------------------------------------------------------------
 ...
 my_cal:set_title_text_color("#777e76")
...
my_text_date = awful.widget.textclock({align = "right"}, '<span background="#5d7bd0" font="Terminus 12"> <span background="#5d7bd0" font="Terminus 9" color="#FFFFFF">%A %B %d, %Y </span></span>', 1)

--{{--| Weather |-----------------------------------------------------------------------------------
function weather_icon(sky)
    time = tonumber(os.date("%H"))
    if time > 19 or time < 6 then
        return weather_night_icon(sky)
    else
        return weather_day_icon(sky)
    end
end
function weather_icon_dep(sky)
    if string.match(sky,'cloudy') == 'cloudy' or string.match(sky,'overcast') == 'overcast' then
        return image(beautiful.widget_cloud)
    elseif string.match(sky,'rain') == 'rain' then
        return image(beautiful.widget_rain)
    elseif string.match(sky,'sun') == 'sun' then
        return image(beautful.widget_sun)
    else
        return image(day_night())
    end
end
function weather_day_icon(sky)
    if string.match(sky,'cloudy') == 'cloudy' or string.match(sky,'overcast') == 'overcast' then
        return image(beautiful.widget_cloud)
    elseif string.match(sky,'rain') == 'rain' then
        return image(beautiful.widget_rain)
    elseif string.match(sky,'sun') == 'sun' then
        ...
        return image(beautiful.widget_sun)
    end
end
function weather_night_icon(sky)
    if string.match(sky,'cloudy') == 'cloudy' or string.match(sky,'overcast') == 'overcast' then
        return image(beautiful.widget_cloud_moon)
    elseif string.match(sky,'rain') == 'rain' then
        return image(beautiful.widget_rain)
    elseif string.match(sky,'sun') == 'sun' then
        return image(beautful.widget_sun)
    else
        return image(beautiful.widget_moon)
    end
end
...
weatherwidget = widget({ type = "textbox" })
    ...
    function (widget, args)
        if args["{tempf}"] == "N/A" then
            return '<span background="#d05d60" font="Terminus 12"> <span background="#d05d60" font="Terminus 9" color="#FFFFFF">No Info </span></span>'
        else
            
            weathericon.image = weather_icon(string.lower(args["{sky}"]))
        --    weatherwidget:add_signal('mouse::enter', function () weather_n = { naughty.notify({ title = "───────────── Weather ─────────────", text = "Wind    : " .. args["{windmph}"] .. " mph " .. args["{wind}"] .. "\nHumidity: " .. args["{humid}"] .. " %\nPressure: " .. args["{press}"] .. " hPa", border_color = "#1a1a1a", timeout = 0, hover_timeout = 0.5 }) } end)
          --  weatherwidget:add_signal('mouse::leave', function () naughty.destroy(weather_n[1]) end)
            return '<span background="#d05d60" font="Terminus 12"> <span background="#d05d60" font="Terminus 9" color="#FFFFFF"> It\'s ' .. string.lower(args["{sky}"]) .. " Nigga, " .. args["{tempf}"] .. "°F </span></span>"
        end
    end, 120, "KSFB" )
...
arr10 = widget ({type = "imagebox"})
arr10.image = image(beautiful.arr10)
arr11 = widget ({type = "imagebox"})
arr11.image = image(beautiful.arr11)
arr12 = widget ({type = "imagebox"})
arr12.image = image(beautiful.arr12)
arr13 = widget ({type = "imagebox"})
arr13.image = image(beautiful.arr13)
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "Escape", function() awful.util.spawn_with_shell("awsetbg -r " .. wall_dir) end),
...
awful.key({ modkey },            "8",        function () awful.util.spawn_with_shell("google-chrome") end),
